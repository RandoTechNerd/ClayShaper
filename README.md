...
**Desktop (Windows)** — download from [Releases](../../releases), unzip, and
double-click `ClayShaper.exe`. It opens in its own window; close the window to
quit. ~10 MB, no install and no Python needed.
...
Five models ship with the app: **Coil Bowl**, **Ripple Cup**, **Belly Vase**,
**Twist Pot** (generated by `web/tools/make_samples.py` — regenerate or tweak
them freely) and **Cuboid**. Drop any `.stl` into `web/assets/models/` and it
appears in the gallery automatically; uploading your own model in the app
always works too.
